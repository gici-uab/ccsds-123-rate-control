...
public class UTQ_URQ implements Quantizer {
	private int quantizationStep;
	private long acumulatedError = 0;
	
	public UTQ_URQ(int quantizationStep) {
	...
	public int quantize(int residual, int t) {
		...
		q_residual=sign*(int)Math.floor(Math.abs(residual)/(float)quantizationStep + 0.5f);
