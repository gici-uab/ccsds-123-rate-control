...
public class BlockAdaptiveCoder extends RiceCoder implements BlockCoder {
	...
	private class SecondExtension {
		...
		public void codeBlock() throws IOException {
			...
			numBitsWritten = unaryCoder.getNumBitsWritten();
		}
	...
	public BlockAdaptiveCoder(BitOutputStream bos, int blockSize, int dynamicRange,
			boolean referenceSamples, int r, int s) {

		super(bos, blockSize, dynamicRange, false);
	...
	public void codeBlock(int[] inBlock) throws IOException {
		...
		if (codingType == ZERO_BLOCK) {
			...
			switch(codingType) {
				...
				case SECOND_EXT :
					secExt.resetNumBitsWritten();
					secExt.codeBlock();
					this.numBitsWritten = this.numBitsWritten + secExt.getNumBitsWritten();
				break;
	...
	void writeId(CodingType type, int k) throws IOException {
		switch(type) {
			...
			case ZERO_BLOCK :
				...
				this.numBitsWritten = this.numBitsWritten + (idBits + 1);
			break;
			case SECOND_EXT :
			...
			case BACKUP :
				...
				this.numBitsWritten = this.numBitsWritten + idBits;
			break;
